<x-jet-dialog-modal maxWidth="2xl" wire:model="isEdit">
    <x-slot name="title">Edit Age Bracket</x-slot>

    <x-slot name="content">
        <form>
            <x-jet-label for="" value="{{ __('Age Bracket Details') }}" />

            <div class="shadow overflow-hidden sm:rounded-md">
                <div class="px-2 py-2 bg-white sm:p-6">

                    <div class="grid grid-cols-1 md:grid-cols-1 gap-6">
                        <div class="mb-4">
                            <x-jet-label for="name" value="{{ __('Name') }}" />

                            <x-jet-input id="name" type="text" class="mt-1 block w-full" wire:model="name" />
                            <x-jet-input-error for="name" class="mt-2" />
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="mb-4">
                            <x-jet-label for="from" value="{{ __('From') }}" />

                            <x-jet-input id="from" type="number" class="mt-1 block w-full" wire:model="from" />
                            <x-jet-input-error for="from" class="mt-2" />
                        </div>
                        <div class="mb-4">
                            <x-jet-label for="to" value="{{ __('To') }}" />

                            <x-jet-input id="to" type="number" class="mt-1 block w-full" wire:model="to" />
                            <x-jet-input-error for="to" class="mt-2" />
                        </div>
                        
                    </div>
                </div>
            </div>
        </form>
    </x-slot>

    <x-slot name="footer">
        <div class="">

            <x-jet-button wire:click="confirmUpdate()">
                {{ __('Save') }}
            </x-jet-button>



            <x-jet-secondary-button wire:click="closeEdit()">
                {{ __('Cancel') }}
            </x-jet-secondary-button>

        </div>
    </x-slot>

</x-jet-dialog-modal>