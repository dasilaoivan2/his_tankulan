...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
            ...
            <h3 class="mb-5 text-lg font-medium text-gray-900 dark:text-white">Please select for Household Type</h3>


            <ul class="grid gap-6 w-full grid-cols-1 md:grid-cols-3">
                @foreach($types as $type)
                <li>
                    <input type="radio" id="{{$type->id}}" wire:model="type_id" value="{{$type->id}}" class="hidden peer" required>
                    <label for="{{$type->id}}" class="inline-flex justify-between items-center p-5 w-full text-gray-500 bg-white rounded-lg border border-gray-200 cursor-pointer dark:hover:text-gray-300 dark:border-gray-700 dark:peer-checked:text-blue-500 peer-checked:border-blue-600 peer-checked:text-blue-600 hover:text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-700">
                        <div class="block">
                            <div class="w-full text-lg font-semibold">{{$type->name}}</div>
                            <div class="w-full">Household Total: {{$type->households->count()}}</div>
                        </div>
